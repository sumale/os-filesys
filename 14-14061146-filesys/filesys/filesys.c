...
int fd_dd(char *directoryName)
{
	...
	pentry = (struct Entry*)malloc(sizeof(struct Entry));
	/*扫描当前目录查找文件*/
	ret = ScanEntry(directoryName,pentry,1);
	if(ret<0)
	{
		printf("no such empty directory\n");
		free(pentry);
		return -1;
	}
	/*清除fat表项*/
...
/*
*参数：directoryName，类型：char
*返回值：1，成功；-1，失败
*功能;删除当前目录下的目录
*/
int fd_dfd(char *directoryName)
{
	int ret, offset,cluster_addr,i,count = 0;
	struct Entry entry;
	struct Entry *dir;
	unsigned char buf[DIR_ENTRY_SIZE];
	char name[100]; 
	...
	int id[100];
	dir = (struct Entry*)malloc(sizeof(struct Entry));
	...
	if( (ret = read(fd,buf,DIR_ENTRY_SIZE))<0)
        ...
        ret = ScanEntry(directoryName,dir,1);
	if(ret < 0){
	...
	while(offset<cluster_addr +CLUSTER_SIZE){
		...
		offset += abs(ret);
		if(ret > 0){
			id[count] = (entry.subdir) ? 1:0;
			strcpy(tempName[count],entry.short_name);
			count++;
		}
	}
	count = count-1;
	if(count == -1){
		strcpy(name,curdir->short_name);			
		fd_cd("..");
		fd_dd(name);
	}
	while(count >= 0){
		if(id[count] == 1){
			fd_dfd(tempName[count]);
			count--;
		}
		else{
			if(fd_df(tempName[count]))
				count--;
		}
		if(count == -1){
			strcpy(name,curdir->short_name);
...
*功能：在当前目录下创建文件
*/
int fd_cf(char *filename,char *astring)
{

	struct Entry *pentry;
	int ret,i=0,j=0,cluster_addr,offset,size=strlen(astring);
...
/*
*参数：directoryName，类型：char，创建目录的名称
*返回值：1，成功；-1，失败
*功能：在当前目录下创建目录
*/
int fd_mkdir(char *directoryName)
{
	...
	if (ret<0)
	{
		...
		if(curdir==NULL)  /*往根目录下写文件*/
		{
			...
			while(offset < DATA_OFFSET)
			{
				...
				{
					offset = offset-abs(ret);
					for(i=0;i<=strlen(directoryName);i++)
					{
						c[i]=toupper(directoryName[i]);
					}
					for(;i<=10;i++)
						c[i]=' ';
...
char* fd_multiCd1(char string[])
{
	char tempAddr[10][100]={0};
	char *addr = (char *)malloc(512);
	int i = 0, j = 1, k = 0, m;
	for(i = 0,j = 1,k = 0;string[j] != '\0';j++){
		if(string[j] != '/'){
			tempAddr[i][k++] = string[j];
		}
		else{
...
int main()
{
	...
	while (1)
	{
		...
		else if(strcmp(input, "df") == 0)
		{
			scanf("%s", name);
			if(name[0] == '/'){
				strcpy(tempName, fd_multiCd1(name));
