...
*功能：显示当前目录的内容
*返回值：1，成功；-1，失败
*funct 为0的时候查看当前目录信息，为1的时候查看所有非根目录信息
*/
int fd_ls(int funct)
{
	...
	if( (ret = read(fd,buf,DIR_ENTRY_SIZE))<0)		//读目录项
		perror("read entry failed");
	int i;
...
int fd_md(char *dirname)
{
	...
	//扫描根目录，是否已存在该文件名
	ret = ScanEntry(dirname,pentry,0);
	if (ret<0)
	{
		...
		{ 
			...
			while(offset < DATA_OFFSET)
			{
				...
				{       
					...
					/*写文件的大小*/
					c[28] = (0x00000000 &  0x000000ff);
					c[29] = ((0x00000000 & 0x0000ff00)>>8);
					c[30] = ((0x00000000& 0x00ff0000)>>16);
					c[31] = ((0x00000000& 0xff000000)>>24);
...
int fd_cf(char *filename,int size)
{
	...
	t = time(NULL);
	local = localtime(&t);
	cftime = local->tm_hour*2048+local->tm_min*32+local->tm_sec/2;
	cfdate = (local->tm_year+1900-1980)*512+(local->tm_mon+1)*32+local->tm_mday;
		...
		{ 
			...
			while(offset < DATA_OFFSET)
			{
				...
				{       
					...
					/*写时间和日期*/
					c[0x16]=(cftime&0x00ff);
					c[0x17]=((cftime&0xff00)>>8);
					c[0x18]=(cfdate&0x00ff);
					c[0x19]=((cfdate&0xff00)>>8);
