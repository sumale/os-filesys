...
void correct_time(char *content){
	time_t nowtime;
	struct tm *timeinfo;
...
void cnew_dir(char *content,int currentStartCluster,int startIndex)
{
	int i = 0;
	...
	correct_time(content+startIndex+22);
...
int fd_df(char *filename)
{
	...
	/*清除目录表项*/
	c=0xe5;
	for(;ret_tt>0;ret_tt-=0x20,ret-=0x20) {
...
int fd_cf(char *filename,char *data)
{
	...
	if (ret<0)
	{
		...
		if(curdir==NULL)  /*往根目录下写文件*/
		{
			...
			while(offset < DATA_OFFSET)
			{
				...
				else
				{
					...
					if(ifdir==0)
						content_cf(clusterno[0],data,size);
					else
					{
						int i;
						char content[640];
						cnew_dir(content,clusterno[0],0);
						cnew_dir(content,0,32);

						content_cf(clusterno[0],content,64);
						ifdir=0;
					}
		...
		else
		{
			...
			cluster_addr = (currentCluster-2 )*CLUSTER_SIZE + DATA_OFFSET;
			if((ret= lseek(fd,cluster_addr,SEEK_SET))<0)
				perror("lseek cluster_addr failed");
			offset = cluster_addr;
			while(offset < cluster_addr + CLUSTER_SIZE)
			{
				...
				else
				{
					...
					if(ifdir!=0){
                                               char content[640];
						cnew_dir(content,clusterno[0],0);
						cnew_dir(content,curdir->FirstCluster,32);
...
int  if_nulldir(char *filename1){  

	struct Entry *pentry1;
	int exit1=0;
	/*扫描当前目录查找文件*/
   //  printf("in");
pentry1= (struct Entry*)malloc(sizeof(struct Entry));

	exit1 = ScanEntry(filename1,pentry1,0);
     
	if(exit1<0)
	{
		exit1=ScanEntry(filename1,pentry1,1);
		if(exit1<0) {
		//printf("no such file\n");
		free(pentry1);
		return 0;
