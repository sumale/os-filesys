...
#define FILESYS_H
#include<stddef.h>
#define DEVNAME "/home/zc/data"//此处填写创建的虚拟文件的地址                         
...
struct Entry *curdir_temp = NULL;
int dirno = 0;/*代表目录的层数*/
struct Entry* fatherdir[10];
int dirno_temp = 0;/*代表目录的层数*/
struct Entry* fatherdir_temp[10];

unsigned char fatbuf[256*512]; 
unsigned char shortname_record[4*512*4/32][12];
int mode_record[4*512*4/32];
int record;
int input_request;
